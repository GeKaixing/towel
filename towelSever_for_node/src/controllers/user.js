...
import fs from "fs";
import { VIPMembership } from "../../abi/VIPMembership.js";
...
export const deactivateaccountAPi = async (req, res) => {
  try {
    ...
    if (!storedCode || storedCode !== code) {
      return res
        .status(400)
        .json({ message: "验证码错误或已过期", status: false });
    }
...
export const modifyingausernameAPi = async (req, res) => {
  try {
    ...
    if (userAndemail) {
      return res.status(400).json({ meassge: "用户名重复哦", status: false });
    }
...
// 修改vip
export const updataPremium = async (req, res) => {
  try {
    const { id, signerAddress } = req.body.data;
    const userAndemail = await USERS.findOne({ _id: id }); //false
    if (!userAndemail) {
      return res.status(400).json({ meassge: "找不到账号", status: false });
    }
    ...
    // 检查该地址是否为 VIP
    const isVIP = await contract.checkVIP(signerAddress);
