  ...
  return (
    <div className='flex justify-center items-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex-col space-y-2'>
      ...
      <form className='flex flex-col justify-center items-center ' onSubmit={(e) => { e.preventDefault(); }}>
        <input required className='w-36 h-6 mt-2.5 bg-[--boxColor] border-0 text-[--fontColor]' type='text' value={valuename_signup} onChange={(e) => { e.preventDefault(); setValuename_signup(e.target.value) }} placeholder='账号' >
        </input>
        <input required type='password' className='w-36 h-6 mt-2.5 bg-boxColor border-0 text-[--fontColor]' value={valuepassword_signup} onChange={(e) => { e.preventDefault(); setValuepassword_signup(e.target.value) }} placeholder='密码'>
        </input>
        <input required type='email' className='w-36 h-6 mt-2.5 bg-boxColor border-0 text-[--fontColor]' value={inputEmailData} onChange={validateEmail} placeholder='电子邮件发送验证码'>
        </input>
        {isEmail ?
          <>
            <input required type='text' minLength={6} maxLength={6} className='w-36 h-6 mt-2.5 bg-boxColor border-0 text-[--fontColor]' value={verificationcode} onChange={(e) => { e.preventDefault(); setVerificationCode(e.target.value) }} placeholder='验证码'>
            </input>
