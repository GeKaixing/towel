  ...
  return (
    <div className='flex justify-center items-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex-col space-y-2'>
      ...
      <form className='flex flex-col justify-center items-center ' onSubmit={(e) => { e.preventDefault(); }}>
        ...
        <input required type='password' className='w-60 h-6 mt-2.5 bg-[--boxColor] border-0 text-[--fontColor]' value={valuepassword_signup} onChange={(e) => { e.preventDefault(); setValuepassword_signup(e.target.value) }} placeholder={t('password')}>
        </input>
        <input required type='email' className='w-60 h-6 mt-2.5 bg-[--boxColor] border-0 text-[--fontColor]' value={inputEmailData} onChange={validateEmail} placeholder={t('EmailTheVerificationCode')}>
        </input>
        {isEmail ?
          <>
            <input required type='text' minLength={5} maxLength={5} className='w-60 h-6 mt-2.5 bg-[--boxColor] border-0 text-[--fontColor]' value={verificationcode} onChange={(e) => { e.preventDefault(); setVerificationCode(e.target.value) }} placeholder={t('Verification')}>
            </input>
