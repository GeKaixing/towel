...
const commentidHandler=(id)=>{commentid.value=id;}
onLoad((option) => {
...
<template>
    ...
    <pageLayout>
        <postInput :id="post.id" :commentid='commentid' @reloadHandler="reloadHandler"></postInput>
        <postComment v-for="(post, index) in commentData" :key="index" :id="post._id" :postId='post.postId'
            :user="post.users" :postText="post.commentText" :postImages="post.commentImages"
